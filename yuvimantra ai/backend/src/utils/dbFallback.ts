...
import { generateChatResponse, analyzeSentiment, generateJournalSummary } from './gemini';
...
export const mockAuth = {
  signup: async (userData: any) => {
    const existing = mockDb.users.find(u => u.email === userData.email.toLowerCase());
  ...
  login: async (credentials: any) => {
    const user = mockDb.users.find(u => u.email === credentials.email.toLowerCase());
...
export const mockHabits = {
  ...
  list: async (userId: string) => {
    return mockDb.habits.filter(h => h.user === userId && !h.isArchived);
  },
  complete: async (userId: string, habitId: string, dateStr: string) => {
    const habit = mockDb.habits.find(h => h._id === habitId && h.user === userId);
  ...
  uncomplete: async (userId: string, habitId: string, dateStr: string) => {
    const habit = mockDb.habits.find(h => h._id === habitId && h.user === userId);
    if (!habit) throw new Error('Habit not found');

    habit.completions = habit.completions.filter((c: string) => c !== dateStr);
    if (habit.streak > 0) habit.streak -= 1;
    return habit;
  },
  delete: async (userId: string, habitId: string) => {
    mockDb.habits = mockDb.habits.filter(h => !(h._id === habitId && h.user === userId));
...
export const mockJournals = {
  create: async (userId: string, data: any) => {
    ...
    const aiSummary = await generateJournalSummary(data.content);
  ...
  list: async (userId: string, query: any) => {
    let list = mockDb.journals.filter(j => j.user === userId);
    if (query.search) {
  ...
  update: async (userId: string, journalId: string, data: any) => {
    const journal = mockDb.journals.find(j => j._id === journalId && j.user === userId);
    if (!journal) throw new Error('Journal not found');

    if (data.title) journal.title = data.title;
    if (data.tags) journal.tags = data.tags;
    if (data.moodEmoji !== undefined) journal.moodEmoji = data.moodEmoji;
    if (data.isFavorite !== undefined) journal.isFavorite = data.isFavorite;
  ...
  delete: async (userId: string, journalId: string) => {
    mockDb.journals = mockDb.journals.filter(j => !(j._id === journalId && j.user === userId));
...
export const mockTasks = {
  ...
  list: async (userId: string) => {
    return mockDb.tasks.filter(t => t.user === userId);
  },
  update: async (userId: string, taskId: string, data: any) => {
    const task = mockDb.tasks.find(t => t._id === taskId && t.user === userId);
    ...
    if (data.dueDate) task.dueDate = new Date(data.dueDate).toISOString();
    if (data.notes !== undefined) task.notes = data.notes;
    if (data.pomodorosExpected !== undefined) task.pomodorosExpected = data.pomodorosExpected;
  ...
  delete: async (userId: string, taskId: string) => {
    mockDb.tasks = mockDb.tasks.filter(t => !(t._id === taskId && t.user === userId));
